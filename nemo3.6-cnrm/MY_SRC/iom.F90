MODULE iom
   ...
   !!--------------------------------------------------------------------
   USE dom_oce, ONLY : nimpp, njmpp, nlci, nlcj, nldi, nldj, nlei, nlej, & 
                       mig, mjg, narea, &
                       gphiv, gphif, &
                       agrif_root, agrif_cfixed, lk_agrif, &
                       rdt,rdttra, gdept_0, gdepw_0,  adatrj, fjulday, &
                       mikt
...
#if defined key_lim3 || defined key_glt
   USE ice    , ONLY :   jpl
   ...
   USE diadct, ONLY : nb_sec, o_sec_names, nb_sec_ice, i_sec_index, o_sec_index, nn_dctwri
#endif
# endif
...
CONTAINS
   ...
   SUBROUTINE iom_init( cdname )
      ...
      INTEGER             :: ji, irefyear, ii,iij
#if defined key_diadct
      CHARACTER(len=35), ALLOCATABLE, DIMENSION(:)   :: o_sec_labs
      CHARACTER(len=35), ALLOCATABLE, DIMENSION(:)   :: i_sec_labs
      INTEGER :: znb_sec_oce
#endif
      !
      ...
      IF (.NOT.(xios_getvar('ref_year',irefyear))) irefyear = 1990
      SELECT CASE ( nleapy )        ! Choose calendar for IOIPSL
      ...
      ! MCH - This block is dirty.
      ! Essentially made to avoid problems.  
      IF( jpni * jpnj == jpnij ) ln_fillcoordinates=.FALSE.
      IF( jpni<0 .OR. jpnj<0   ) ln_fillcoordinates=.FALSE.
      IF( lk_agrif             ) ln_fillcoordinates=.FALSE.
      INQUIRE(FILE='coordinates_xios.nc', EXIST=file_exist)
      IF( .NOT. using_xios_coordinates .AND. file_exist ) using_xios_coordinates=.TRUE.
      ! MCH - End

      IF( TRIM(cdname) == TRIM(cxios_context) ) THEN  
         IF ( ln_fillcoordinates .AND. using_xios_coordinates ) THEN
           ...
           CALL dom_uniq_xios( zmaskutil, 'T' )
           DO jk = 1, jpkm1
              DO jj = 1, jpj
                 DO ji = 1, jpi
                    ikt = mikt(ji,jj)
                    zmask(ji,jj,jk) = tmask(ji,jj,jk) * tmask(ji,jj,ikt) * zmaskutil(ji,jj)
                 END DO
              END DO
           END DO
           ...
           zmask=0 ; zmask_xios=.FALSE.
           CALL dom_uniq_xios( zmaskutil, 'U' )
           DO jk = 1, jpkm1
              DO jj = 1, jpj
                 DO ji = 1, jpi
                    ikt = miku(ji,jj)
                    zmask(ji,jj,jk) = umask(ji,jj,jk) * umask(ji,jj,ikt) * zmaskutil(ji,jj)
                 END DO
              END DO
           END DO
!           zmask(2:jpim1,:,:) = tmask(2:jpim1,:,:) + tmask(3:jpi,:,:)   ;   CALL lbc_lnk( zmask, 'U', 1. ) 
           ...
           zmask=0 ; zmask_xios=.FALSE.
           CALL dom_uniq_xios( zmaskutil, 'V' )
           DO jk = 1, jpkm1
              DO jj = 1, jpj
                 DO ji = 1, jpi
                    ikt = mikv(ji,jj)
                    zmask(ji,jj,jk) = vmask(ji,jj,jk) * vmask(ji,jj,ikt) * zmaskutil(ji,jj)
                 END DO
              END DO
           END DO
!           zmask(:,2:jpjm1,:) = tmask(:,2:jpjm1,:) + tmask(:,3:jpj,:)   ;   CALL lbc_lnk( zmask, 'V', 1. )
           ...
           DO jk = 1, jpkm1
              DO jj = 1, jpj
                 DO ji = 1, jpi
                    ikt = mikt(ji,jj)
                    zmask(ji,jj,jk) = wmask(ji,jj,jk) * tmask(ji,jj,ikt) * zmaskutil(ji,jj)
                 END DO
              END DO
           END DO
...
# endif
#if defined key_lim3 || defined key_lim2 || defined key_glt
      ...
      CALL iom_set_axis_attr( "allbasin"  , (/ (REAL(ji,wp), ji=1,5) /) )
!      CALL iom_set_axis_attr( "3basin" , (/ (REAL(ji,wp), ji=1,3) /) )
...
#if defined key_diadct
      znb_sec_oce =  nb_sec - nb_sec_ice
      ALLOCATE( o_sec_labs( znb_sec_oce ) )
      ALLOCATE( i_sec_labs( nb_sec_ice ) )
      o_sec_labs=(/("", ii=1,znb_sec_oce)/)    
      i_sec_labs=(/("", ii=1, nb_sec_ice)/)    
      DO iij=1,znb_sec_oce
        WRITE(o_sec_labs(iij),*), TRIM(o_sec_names(o_sec_index(iij)))
      END DO
      DO iij=1,nb_sec_ice
        WRITE(i_sec_labs(iij),*), TRIM(o_sec_names(i_sec_index(iij)))
      END DO

      ! definition of axis "oline" -- ocean sections for diadct
      IF (xios_is_valid_axis("oline")) THEN
        CALL xios_set_axis_attr("oline",n_glo=znb_sec_oce)
        IF(lwp) WRITE(numout,*),'initializing labels for oline to:',o_sec_labs
        CALL xios_set_axis_attr("oline",value=(/(REAL(ii,wp), ii=1,znb_sec_oce)/))
        CALL xios_set_axis_attr("oline",label=o_sec_labs)
        CALL xios_set_axis_attr("oline",name="oline")
        CALL xios_set_axis_attr("oline",standard_name="Sections")
        CALL xios_set_axis_attr("oline",long_name="ocean passage")
      ENDIF

      IF(lwp) WRITE(numout,*),'      '

      ! definition of axis "siline" -- ice sections for diadct
      IF (xios_is_valid_axis("siline")) THEN
        IF ( nb_sec_ice .GT. 0) THEN
           CALL xios_set_axis_attr("siline",n_glo=nb_sec_ice)
           IF(lwp) WRITE(numout,*),'initializing labels for siline to:',i_sec_labs
           CALL xios_set_axis_attr("siline",value=(/(REAL(ii,wp), ii=1,nb_sec_ice)/))
           CALL xios_set_axis_attr("siline",label=i_sec_labs)
           CALL xios_set_axis_attr("siline",name="siline")
           CALL xios_set_axis_attr("siline",standard_name="Ice Sections")
           CALL xios_set_axis_attr("siline",long_name="ocean passage with sea ice")
        ENDIF
      ENDIF
      DEALLOCATE( o_sec_labs )
      DEALLOCATE( i_sec_labs )
#else
      ! definition of dummy oline and iline should be done for compatibility
      CALL iom_set_axis_attr("oline", (/ REAL(1,wp) /) )
      CALL iom_set_axis_attr("siline", (/ REAL(1,wp) /) )
#endif

      CALL xios_set_axis_attr("jmean",n_glo=jpjglo, begin=n_jbegin-1, n=n_nj, value=(/(REAL(ii,wp), ii=n_jbegin,n_jbegin-1+n_nj)/))
      ! definition of axis "basin" -- dirty and not OMIP-compliant
      !CALL xios_set_axis_attr("basin",n_glo=5)
      !CALL xios_set_axis_attr("basin",value=(/(REAL(ii,wp), ii=1,5)/))
      !CALL xios_set_axis_attr("basin",label=(/ 'global_ocean', &
      !          & 'atlantic_ocean','pacific_ocean','indian_ocean',&
      !          & 'indian-pacific_ocean' /) )
      !CALL xios_set_axis_attr("basin",name="basin")
      !CALL xios_set_axis_attr("basin",standard_name="Basins")
      !CALL xios_set_axis_attr("basin",standard_name="Ocean region masks")

      ! end file definition
      dtime%second = rdt
      CALL xios_set_timestep(dtime)
      IF( jpni*jpnj == jpnij .AND. .NOT. using_xios_coordinates .AND. .NOT. lk_agrif ) CALL dom_xios_create_coordinates
      lxios = xios_setvar('rau0',rau0)
      lxios = xios_setvar('rcp',rcp)
            ...
            ELSE 
               IF( .NOT. PRESENT(pv_r1d) ) THEN   !   not a 1D array
                  IF(     idom == jpdom_data    ) THEN
                     ...
                     ENDIF
                     IF( lk_crs .AND. jpi == jpi_crs .AND. jpi .NE. 0  )THEN
                        istart(1:2) = (/ mig_crs(1), mjg_crs(1) + jstartrow - 1 /)  ! icnt(1:2) done below
                     ELSE
...
!         ENDIF
         ...
         IF( istop == nstop ) THEN   ! no additional errors until this point...
            ...
            IF(     PRESENT(pv_r2d) .AND. idom /= jpdom_unknown ) THEN
               IF( lk_crs .AND. jpi == jpi_crs .AND. jpi .NE. 0 )THEN
                  CALL crs_lbc_lnk( pv_r2d,'Z',-999.,'no0')
               ELSE
            ...
            ELSEIF( PRESENT(pv_r3d) .AND. idom /= jpdom_unknown ) THEN
               ...
               IF( icnt(3) == jpk ) THEN
                  IF( lk_crs .AND. jpi == jpi_crs .AND. jpi .NE. 0 )THEN
                     CALL crs_lbc_lnk( pv_r3d,'Z',-999.,'no0')
                  ELSE
...
!CDIR COLLAPSE
     ...
     ENDIF
      ...
      IF ( xios_is_valid_domain     (cdid) ) THEN
         CALL xios_set_domain_attr     ( cdid, ni_glo=ni_glo, nj_glo=nj_glo, ibegin=ibegin, jbegin=jbegin, ni=ni, nj=nj,   &
            ...
            &    bounds_lat_1D=bounds_lat, type='curvilinear')
     ENDIF
      IF ( xios_is_valid_domaingroup(cdid) ) THEN
         CALL xios_set_domaingroup_attr( cdid, ni_glo=ni_glo, nj_glo=nj_glo, ibegin=ibegin, jbegin=jbegin, ni=ni, nj=nj,   &
            &    data_dim=data_dim, data_ibegin=data_ibegin, data_ni=data_ni, data_jbegin=data_jbegin, data_nj=data_nj ,   &
  ...
  END SUBROUTINE iom_set_zoom_domain_attr
   ...
   SUBROUTINE iom_setkt( kt, cdname )
      ...
      IF( jpni*jpnj == jpnij .AND. .NOT. using_xios_coordinates .AND. .NOT. lk_agrif ) CALL dom_xios_write_coordinates
   ...
   SUBROUTINE set_grid_xios( grid_name, lon, lat, bounds_lon, bounds_lat, area, mask )
     ...
     CALL iom_set_domain_attr(grid_name, ni_glo=jpiglo, nj_glo=jpjglo, ibegin=n_ibegin-1, jbegin=n_jbegin-1, ni=n_ni, nj=n_nj)
      ...
      CALL iom_set_domain_attr(grid_name, nvertex=4)
     ...
     CALL iom_set_domain_attr("grid_"//cdgrd, ni_glo=jpiglo, nj_glo=jpjglo, ibegin=nimpp+nldi-2, jbegin=njmpp+nldj-2, ni=ni, nj=nj)
      ...
      IF ( ln_mskland ) THEN
         ...
         IF( lk_crs .AND. jpi == jpi_crs .AND. jpi .NE. 0  )THEN
            ...
            SELECT CASE ( cdgrd )
            CASE('T')   ;   zmask(:,:,:)       = tmask_crs(:,:,:)
            CASE('U')   ;   zmask(2:jpim1,:,:) = tmask_crs(2:jpim1,:,:) + tmask_crs(3:jpi,:,:)   ;   CALL crs_lbc_lnk( zmask, 'U', 1. )
            CASE('V')   ;   zmask(:,2:jpjm1,:) = tmask_crs(:,2:jpjm1,:) + tmask_crs(:,3:jpj,:)   ;   CALL crs_lbc_lnk( zmask, 'V', 1. )
            CASE('W')   ;   zmask(:,:,2:jpk  ) = tmask_crs(:,:,1:jpkm1) + tmask_crs(:,:,2:jpk)   ;   zmask(:,:,1) = tmask_crs(:,:,1)
            END SELECT
   ...
   SUBROUTINE set_grid_znl( plat )
      ...
      CALL dom_ngb( -168.5377, 65.03584, ix, iy, 'T' ) !  i-line that passes in Bering Strait : Reference latitude (used in plots)
...
# if defined key_diadct
      f_op%timestep = nn_dctwri ;  f_of%timestep = 0  ; CALL iom_set_field_attr('diadct'          , freq_op=f_op, freq_offset=f_of)
# endif
#endif
...
!  USE xios
  ...
  TYPE(xios_field) :: hfield 
    ...
    CALL xios_set_attr(hfile,name="coordinates_xios",enabled=.TRUE., output_freq=xios_timestep)
...
! write coordinates_xios.nc file
  ...
  LOGICAL,SAVE :: first=.TRUE.
    ...
    IF (first .AND. .NOT. using_xios_coordinates) THEN
      ...
      CALL iom_put("dom_xios_field_U",umask(:,:,1))
      CALL iom_put("dom_xios_field_V",vmask(:,:,1))
      first=.FALSE.
    ENDIF
#endif
