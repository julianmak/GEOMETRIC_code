MODULE ldfeke
   ...
   USE ldftra_oce     !
   USE dynldf_lap, ONLY : eke_keS   ! source term of eke due to KE dissipation
   USE zdfmxl         ! vertical physics: mixed layer
   
   USE dynspg_oce, ONLY: un_adv, vn_adv
...
CONTAINS
   ...
   SUBROUTINE ldf_eke( kt )
      ...
      !!
      !! ** Notes   :   If lk_ldfeke = true then eke and aeiw are BOTH updated
      !!
      ...
      DO jj = 1, jpjm1
         DO ji = 1, fs_jpim1   ! vector opt.
            ! upstream scheme
            zfp_ui = un_adv(ji,jj) + ABS( un_adv(ji,jj) ) ! jm (28 Oct 18): DO NOT use filtered free surface (removed in NEMO 4.0)
            zfm_ui = un_adv(ji,jj) - ABS( un_adv(ji,jj) ) !                 because then un_adv is not calculated
      ...
      DO jj = 2, jpjm1
         ...
         END DO
      END DO
! jm (28 Oct 18): linear free surface flag is a cpp_key so not bothering with this here
...
!      ENDIF
      ...
      IF ( ln_adv_wav ) THEN
            ...
            DO ji = 2, fs_jpim1
               ...
               IF ( ht_0(ji,jj) > 300._wp ) THEN   ! jm: should use something like ht_b really...
                  ...
                  zc_ros(ji,jj) =  zc1(ji,jj) * zc1(ji,jj) * COS( rad * gphit(ji,jj) )   &
                                / (  ra * ff(ji,jj) * SIN( rad * gphit(ji,jj) )            &
                                + rsmall  )
         ...
         DO jj = 2, jpjm1
            DO ji = fs_2, fs_jpim1   ! vector opt.
               ...
               zadv_wav(ji,jj) = zadv_wav(ji,jj) * MIN(  1._wp, ABS( ff(ji,jj) * z1_f20 )  )     ! tropical decrease
            END DO
         END DO
      END IF
      !
      ...
      IF ( rn_eke_lap >= 0._wp ) THEN
         DO jj = 1, jpjm1
            DO ji = 1, fs_jpim1   ! vector opt.
               zaheeu(ji,jj) = rn_eke_lap * re2u_e1u(ji,jj) * umask(ji,jj,1)   ! rn_eke_lap is constant (for now) and NOT masked
               zaheev(ji,jj) = rn_eke_lap * re1v_e2v(ji,jj) * vmask(ji,jj,1)   !      before it is pahu and pahv which IS maked
            END DO
         END DO
      ...
      DO jj = 2, jpjm1
         DO ji = fs_2, fs_jpim1   ! vector opt.
            ...
            zaeiw(ji,jj,1) = zaeiw(ji,jj,1) * MIN(  1._wp, ABS( ff(ji,jj) * z1_f20 )  )            ! tropical decrease
      ...
      CALL iom_put( "trd_eke_keS"    ,  eke_keS * ssmask )      ! KE to EKE source trend       (RHS)
   ...
   SUBROUTINE ldf_eke_init
      ...
      NAMELIST/namldf_eke/  rn_ekedis  , nn_eke_dis ,   & ! GEOM master params (lambda and option, alpha, e0)
         ...
         &                  ln_adv_wav                    ! options for advection by Rossby wave
      !!----------------------------------------------------------------------
      !
      !
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namldf_eke in configuration namelist', lwp )
      ...
      IF(lwp) THEN                    !* Control print
         ...
         WRITE(numout,*)    '      aeiw updated according to GEOMETRIC             lk_ldfeke   = ', lk_ldfeke
