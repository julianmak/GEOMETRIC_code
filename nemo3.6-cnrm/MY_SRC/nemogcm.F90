MODULE nemogcm
   ...
   USE ldftra_crs      ! lateral diffusivity setting    (ldftra_init routine)
   ...
   USE diaar5          ! AR5 diagnosics                (dia_ar5_init routine)  ! 2017 SB_NEMOCM
   ...
   !! $Id: nemogcm.F90 7332 2016-11-25 08:38:58Z cbricaud $
...
# endif
      ...
      IF(lwp) WRITE(numout,cform_aaa)   ! Flag AAAAAAA
      !
      IF( nstop /= 0 .AND. lwp ) THEN   ! error print  ! 2017 SB_NEMOCM
         cmd='touch nemo_NOK'
         CALL system(cmd)
...
#endif
#if defined key_glt  
!     ne fonctionne pas si on a GELATO     ! 2017 SB_NEMOCM
      ln_ctl=.false.                       ! 2017 SB_NEMOCM
#endif              
      !
      !                             !--------------------------------------------!
      ...
      IF( lk_crs        )   CALL     crs_init   ! Domain initialization of coarsened grid
      ...
      !     
      IF( lk_crs        )   CALL     eos_init_crs   ! Equation of state
      !
                                ! Ocean physics
      !                                         ! Vertical physics
      ...
      IF( lk_ldfeke     )   CALL ldf_eke_init      ! GEOMETRIC param.
      !
      IF( ln_crs_top .AND. lk_ldfslp ) THEN
                            CALL dom_grid_crs
                            CALL ldf_slp_init_crs
                            CALL dom_grid_glo
      ENDIF
      !                                     ! Active tracers
      ...
      !
      !cbr IF( ln_crs_top .AND. lk_trabbl     )  THEN
      !                      CALL dom_grid_crs 
      !                      CALL tra_bbl_init_crs   ! advective (and/or diffusive) bottom boundary layer scheme
      !                      CALL dom_grid_glo
      !ENDIF
      !
      ...
      IF( ln_crs_top )      CALL dom_grid_crs
                            CALL     trc_init
      IF( ln_crs_top )      CALL ldf_tra_crs_init
      IF( ln_crs_top )      CALL dom_grid_glo
#endif
      !                                     ! Diagnostics
      IF( lk_floats     )   CALL     flo_init   ! drifting Floats
!      IF( lk_diaar5     )   CALL dia_ar5_init   ! ar5 diag - TEMPORARY REMOVED
   ...
   SUBROUTINE nemo_alloc
      ...
      ierr = ierr + zdf_mxl_alloc   ()          ! mixed layer
      !
