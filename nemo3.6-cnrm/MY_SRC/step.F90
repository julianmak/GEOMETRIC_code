...
!!gm   #  include "zdfddm_substitute.h90"
   ...
   !! $Id: step.F90 7256 2016-11-18 07:18:45Z cbricaud $
...
CONTAINS
   ...
   SUBROUTINE stp( kstp )
      ...
      REAL(wp), POINTER, DIMENSION(:,:,:) ::   zrhop,zrhd
...
# endif
      ...
      IF( kstp == nit000 ) THEN
         ...
         IF( lk_crs ) CALL iom_init( TRIM(cxios_context)//"_crs" )  ! initialize context for coarse grid
      ENDIF

      IF( kstp /= nit000 )   CALL day( kstp )         ! Calendar (day was already called at nit000 in day_init)
                             CALL iom_setkt( kstp - nit000 + 1,      cxios_context          )   ! tell iom we are at time step kstp
      IF( lk_crs     )       CALL iom_setkt( kstp - nit000 + 1, TRIM(cxios_context)//"_crs" )   ! tell iom we are at time step kstp

      !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
      ! Update data, open boundaries, surface boundary condition (including sea-ice)
      !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
#if ! defined key_glt_offline
...
! jm (28 Oct 18): GEOMETRIC here because it needs un_adv which is not defined until dynspg is called      
#if defined key_traldf_c3d
      IF( lk_ldfeke   )     CALL ldf_eke      ( kstp )  ! GEOMETRIC param. (time evolution of eiv coefficient)
#endif
      ...
      IF( lk_diaharm )      CALL dia_harm( kstp )        ! Tidal harmonic analysis
                            CALL dia_prod( kstp )        ! ocean model: product diagnostics ! 2017 SB_NEMOCM
                            CALL dia_wri( kstp )         ! ocean model: outputs
      !
      IF( lk_crs     )      CALL crs_fld( kstp )         ! ocean model: online field coarsening & output

      CALL wrk_alloc( jpi, jpj, jpk, zrhop, zrhd )
      CALL eos( tsn, zrhd, zrhop, fsdept_n(:,:,:) )                 ! now in situ and potential density
      zrhop(:,:,jpk) = 0._wp
      CALL iom_put( 'rhop', zrhop )

      CALL wrk_dealloc( jpi, jpj, jpk, zrhop, zrhd )
      ...
      IF( lk_crs )   THEN
                         ...
                         CALL zdf_mxl_crs(kstp)
                         CALL eos_crs ( tsn_crs, rhd_crs, rhop_crs, fsdept_crs(:,:,:) ) ! now in situ density for hpg computation

         IF( ln_zps )    CALL zps_hde_crs( kstp, jpts, tsb_crs, gtsu_crs, gtsv_crs, rhd_crs, gru_crs, grv_crs )

         IF( lk_ldfslp .AND.  .NOT. ln_traldf_grif )  &
                         CALL ldf_slp_crs( kstp, rhd_crs, rb2_crs )
                         CALL dom_grid_glo

      ENDIF

      IF( ln_crs_top )   CALL dom_grid_crs

                         CALL trc_stp( kstp )         ! time-stepping

      IF( ln_crs_top )   CALL dom_grid_glo
...
#else                                                                                          
                                                            ! key_glt_offline case                  ! 2017 SB_NEMOCM
                         CALL sbc    ( kstp )         ! Sea Boundary Condition (including sea-ice)  ! 2017 SB_NEMOCM
#endif                                                                                        
      !
#if defined key_iomput
      IF( kstp == nitend .OR. indic < 0 ) THEN 
                      CALL iom_context_finalize(      cxios_context          ) ! needed for XIOS+AGRIF
         IF( lk_crs ) CALL iom_context_finalize( trim(cxios_context)//"_crs" ) ! 
      ENDIF
#endif
      !
