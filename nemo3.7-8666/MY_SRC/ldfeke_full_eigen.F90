MODULE ldfeke
   ...
   INTEGER  ::   nn_wav_cal = 360           !  number of time steps between eigenvalue calculation
   !
   ...
   REAL(wp), ALLOCATABLE, SAVE, DIMENSION(:,:) ::   zc1, zc_ros, zadv_wav ! 1st baroclinic mode and long Rossby speed [m /s ]
                                                                          ! zadv_wave defined here because generically
                                                                          !   nn_wav_cal > 1, otherwise the variable
                                                                          !   is not static
...
!      ENDIF
      ...
      zadv_wav(:,:) = 0._wp
      IF ( ln_adv_wav ) THEN
         ! only update tendency every so often because eigenvalue calculations can be slow
         IF( MOD( kt-1, nn_wav_cal ) == 0 ) THEN
            !
            DO jj = 2, jpjm1 ! first work out the Rossby phase speeds
               DO ji = 2, fs_jpim1
                  ...
                  IF ( ht_0(ji,jj) > 300._wp ) THEN   ! jm: should use something like ht_b really...
                     ...
                     CALL eke_rossby(  e3t_b(ji,jj,:) * tmask(ji,jj,:),   &
                        &              e3w_b(ji,jj,:) * wmask(ji,jj,:),   &
                        &              rn2b(ji,jj,:), mbkt(ji,jj), zc1(ji,jj)  )
                     zc1(ji,jj) = MIN( 10._wp, zc1(ji,jj) )
...
!!
   ...
   SUBROUTINE eke_rossby( z_e3t, z_e3w, zn2, k_bot, zc1 )
     !!---------------------------------------------------------------------
     !!                   ***  ROUTINE eke_rossby  ***
     !!                     
     !! ** Purpose :   Calculate the phase speed associated with the gravest
     !!              baroclinic mode
     !!
     !! ** Method  :   Using a power iteration method with shifts to converge
     !!              to the eigenvalue of the generalised eigenvalue problem
     !!
     !!                    w'' + (N^2 / c^2) w = 0,  w(1) = w(kz) = 0
     !!
     !!              closest to some initial guess. The system is tridiagonal
     !!              via a centred difference in the vertical, so matrix
     !!              multiplications with the inverse is done using a Thomas
     !!              algorithm (in the subroutine eke_thomas)
     !!----------------------------------------------------------------------

      REAL(wp), DIMENSION(jpk) ::   z_e3t, z_e3w, zn2   ! 1D inputs
      INTEGER                  ::   k_bot               ! 0D input
      REAL(wp)                 ::   zc1                ! 0D in/output

      REAL(wp)                 ::   ztol = 1e-6         ! loop variable
      INTEGER                  ::   ik   , iter
      REAL(wp)                 ::   zsig , zmu  , zcb   ! 0D workspace
      REAL(wp), DIMENSION(jpk) ::   zmd  , zld  , zud   ! 1D workspace
      REAL(wp), DIMENSION(jpk) ::   zin  , zout         ! -     -
      
      zsig = MAX(  1 / (zc1 * zc1), 1e-16  )
      ...
      DO ik = 2,k_bot
         zmd(ik) =   1._wp / (z_e3t(ik-1) * z_e3w(ik))   &
            &      + 1._wp / (z_e3t(ik  ) * z_e3w(ik))   &
            &      - zsig * zn2(ik)
         zld(ik) = - 1._wp / (z_e3t(ik-1) * z_e3w(ik))
         zud(ik) = - 1._wp / (z_e3t(ik  ) * z_e3w(ik))
      END DO
      zld(1)       = 0._wp
      zud(k_bot)   = 0._wp
      ...
      CALL eke_thomas( zmd, zld, zud, zn2 * zin, zout, k_bot)

      zmu = SUM( zout(2:k_bot) * zin(2:k_bot) )
      zc1 = 1._wp / SQRT( ABS(zsig + 1._wp / zmu) )
      zcb = 1.e10
      
      iter = 1
      DO WHILE (  abs( zc1 - zcb ) > ztol .AND. iter < 20  )
         zin(2:k_bot) = zout(2:k_bot)   &
            &         / SQRT(  SUM( zout(2:k_bot) * zout(2:k_bot) )  )
         CALL eke_thomas( zmd, zld, zud, zn2 *  zin, zout, k_bot )
         zmu  = SUM( zout(2:k_bot) * zin(2:k_bot) )
   ...
   END SUBROUTINE eke_rossby
!   
   SUBROUTINE eke_thomas( zmd, zld, zud, zin, zout, k_bot )
      !!---------------------------------------------------------------------
      !!                   ***  ROUTINE eke_thomas  ***
      !!                     
      !! ** Purpose :   solve for a linear tridiagonal system (used in 
      !!              eke_rossby), with inputs as vectors
      !!
      !! ** Method  :   standard Thomas (backward substitution) algorithm
      !!----------------------------------------------------------------------

      REAL(wp), DIMENSION(jpk) ::   zmd, zld, zud, zin   ! 1D inputs
      REAL(wp), DIMENSION(jpk) ::   zout                 ! 1D output

      INTEGER  ::   k_bot, jk      ! dummy loop arguments
      REAL(wp), DIMENSION(jpk) :: zc, zd
      ...
      ! forward sweep, from 1 below surface to the last wet point
      zc(2) = zud(2) / zmd(2)
      zd(2) = zin(2) / zmd(2)

      DO jk = 3, k_bot
         zc(jk) = zud(jk) / (zmd(jk) - zld(jk) * zc(jk-1))
         zd(jk) = (  zin(jk) - zld(jk) * zd(jk-1)  )    &
            &   / (  zmd(jk) - zld(jk) * zc(jk-1)  )
      END DO

      zd(k_bot) = (  zin(k_bot) - zld(k_bot) * zd(k_bot-1)  )   &
         &      / (  zmd(k_bot) - zld(k_bot) * zc(k_bot-1)  )

      ! backward substitution
      zout(k_bot) = zd(k_bot)
      DO jk = k_bot-1, 2, -1
         zout(jk) = zd(jk) - zc(jk) * zout(jk+1)
      END DO
      
   END SUBROUTINE eke_thomas
   ...
   SUBROUTINE ldf_eke_init
      ...
      NAMELIST/namldf_eke/  rn_ekedis  , nn_eke_dis ,   & ! GEOM master params (lambda and option, alpha, e0)
         ...
         &                  ln_adv_wav , nn_wav_cal       ! options for advection by Rossby wave
      !!----------------------------------------------------------------------
      !
      !
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namldf_eke in configuration namelist', lwp )
      ...
      IF(lwp) THEN                    !* Control print
         ...
         WRITE(numout,*)    '         number of time steps between eig calc        nn_wav_cal  =  ', nn_wav_cal
      ENDIF
      !                                ! allocate eke arrays
      ...
      zadv_wav(:,:) = 0._wp
      IF ( ln_adv_wav ) THEN
         IF(lwp) WRITE(numout,*) '      number of time steps between eig calc           nn_wav_cal  =  ', nn_wav_cal
         DO jj = 2, jpjm1
            DO ji = 1, fs_jpim1
