...
!!jm hack 22 Jun 17: to amplify the wind stress only
   REAL(wp), ALLOCATABLE, SAVE, DIMENSION(:,:) ::   z_ampu   ! amplify the wind stress at u points
   REAL(wp), ALLOCATABLE, SAVE, DIMENSION(:,:) ::   z_ampt   ! amplify the wind stress at t points
!!
...
CONTAINS
   ...
   SUBROUTINE sbc_blk_init
      ...
      INTEGER  ::   inum                  ! for reading wind stress amplification file
      !!
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namsbc_blk in configuration namelist', lwp )
      ...
      ENDIF
!!jm hack 22 Jun 17: to amplify the wind stress only
      ALLOCATE( z_ampu(jpi,jpj)   , z_ampt(jpi,jpj) )
      CALL iom_open ( 'wind_stress_amp.nc', inum )
      CALL iom_get  ( inum, jpdom_data, 'zampu', z_ampu ) ! read in mask
      CALL iom_get  ( inum, jpdom_data, 'zampt', z_ampt ) ! read in mask
      CALL iom_close( inum )
!!
      !
...
!!gm    move zwnd_i (_j) set to zero  inside the key_cyclone ???
      ...
      DO jj = 1, jpj             ! tau module, i and j component
         DO ji = 1, jpi
            ...
            taum  (ji,jj) = zztmp * wndm  (ji,jj) * z_ampt(ji,jj)
      ...
      DO jj = 1, jpjm1
         DO ji = 1, fs_jpim1
            utau(ji,jj) = 0.5 * ( 2. - umask(ji,jj,1) ) * ( zwnd_i(ji,jj) + zwnd_i(ji+1,jj  ) ) &
               &          * MAX(tmask(ji,jj,1),tmask(ji+1,jj,1)) * z_ampu(ji,jj)
      ...
      CASE( 'I' )                  ! B-grid ice dynamics :   I-point (i.e. F-point with sea-ice indexation)
         ...
         DO jj = 2, jpjm1
            DO ji = 2, jpim1   ! B grid : NO vector opt
               ...
               utau_ice(ji,jj) = zwnorm_f * zwndi_f * z_ampu(ji,jj)
               ...
               wndm_ice(ji,jj) = SQRT( zwndi_t * zwndi_t + zwndj_t * zwndj_t ) * tmask(ji,jj,1) * z_ampt(ji,jj)
            END DO
         END DO
         ...
         DO jj = 2, jpjm1
            DO ji = fs_2, fs_jpim1   ! vect. opt.
               utau_ice(ji,jj) = 0.5 * zrhoa(ji,jj) * Cd(ji,jj) * ( wndm_ice(ji+1,jj  ) + wndm_ice(ji,jj) )                          &
                  &          * ( 0.5 * (sf(jp_wndi)%fnow(ji+1,jj,1) + sf(jp_wndi)%fnow(ji,jj,1) ) - rn_vfac * u_ice(ji,jj) ) * z_ampu(ji,jj)
