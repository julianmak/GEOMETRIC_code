...
CONTAINS
   ...
   SUBROUTINE ldf_eke( kt )
      ...
      REAL(wp), DIMENSION(jpi,jpj)     ::   zubar, zvbar                     !  -     - 
...
! JM: un_adv is the depth-INTEGRATED velocity? GEOMETRIC uses depth-averaged
      ...
      zubar(:,:) = un_adv(:,:) * r1_hu_n(:,:)
      zvbar(:,:) = vn_adv(:,:) * r1_hv_n(:,:)
      !
      DO jj = 1, jpjm1
         DO ji = 1, fs_jpim1   ! vector opt.
            ! upstream scheme
            zfp_ui = zubar(ji,jj) + ABS( zubar(ji,jj) )
            zfm_ui = zubar(ji,jj) - ABS( zubar(ji,jj) )
            zfp_vj = zvbar(ji,jj) + ABS( zvbar(ji,jj) )
            zfm_vj = zvbar(ji,jj) - ABS( zvbar(ji,jj) )
            zwx(ji,jj) = 0.5 * e2u(ji,jj) * hu_0(ji,jj) * ( zfp_ui * zeke_ht(ji,jj) + zfm_ui * zeke_ht(ji+1,jj  ) )
            zwy(ji,jj) = 0.5 * e1v(ji,jj) * hv_0(ji,jj) * ( zfp_vj * zeke_ht(ji,jj) + zfm_vj * zeke_ht(ji  ,jj+1) )
         END DO
      END DO
...
!         DO jj = 2, jpjm1
!            DO ji = fs_2, fs_jpim1   ! vector opt.
!               zadv_ubt(ji,jj) = zadv_ubt(ji,jj) - wn(ji,jj,1) * eke_b(ji,jj) / ( ht_0(ji,jj) + 1._wp-tmask(ji,jj,1) )   ! jm (03 Mar 18): was eke_n
!            END DO
!         END DO
!      ENDIF
      !
      ...
      DO jj = 2, jpjm1
         ...
         END DO
      END DO
      CALL lbc_lnk( 'ldfeke', eke_a, 'T', 1. )  ! Lateral boundary conditions on zwi  (unchanged sign)
      ...
      IF( l_eke_eiv ) THEN
         ...
         DO jj = 2, jpjm1
            ...
            END DO
         END DO
         CALL lbc_lnk( 'ldfeke', zaeiw(:,:,1), 'W', 1. )
         !
         !                          ! inner value
         ...
         END DO 
         !                    !--  diagnostics  --!
         CALL lbc_lnk_multi( 'ldfeke', aeiu , 'U', 1. , aeiv , 'V', 1. )      ! lateral boundary condition
         CALL lbc_lnk( 'ldfeke', zross, 'W', 1. )
      END IF
      !
      IF( lrst_oce )   CALL eke_rst( kt, 'WRITE' )
      
      !                    !==  output EKE related variables  ==!
      CALL lbc_lnk_multi( 'ldfeke',                                                 &   ! lateral boundary condition
         &                zadv_ubt, 'T', 1. , zadv_wav, 'T', 1. , zdis, 'T', 1. ,   &
         &                zlap    , 'T', 1. , zeke_peS, 'T', 1. ,                   & 
         &                zc1     , 'W', 1. , zc_ros  , 'W', 1. )      
      !
